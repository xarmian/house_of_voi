<script lang="ts">
  ...
  async function copyReplayLink(spin: PlayerSpin) {
    try {
      const url = `${window.location.origin}/replay?tx=${encodeURIComponent(formatTxIdForDisplay(spin.txid,58))}`;
      ...
      toastStore.error('Failed to create replay link', 'Please try again', 5000);
    }
  }

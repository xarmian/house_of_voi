...
export const load = async ({ url }) => {
  ...
  try {
    // Transaction ID path (preferred - shortest URLs)
    if (txId) {
      const transactionData = await extractSpinDataFromTransaction(txId);
      if (!transactionData) {
        return {
          pageMetaTags: { title: 'Invalid Transaction - House of Voi', description: 'Transaction not found or invalid' },
      ...
      const pageMetaTags = {
        ...
        description: `View detailed spin verification from transaction ${txId.slice(0, 8)}...`
      } satisfies MetaTagsProps;
